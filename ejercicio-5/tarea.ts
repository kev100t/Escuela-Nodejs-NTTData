...
(() => {
	...
	const obtenerInfo = (
		...
		console.log(
			`El nombre del usuario es ${nombre}${apellidoText}con edad de ${edad} y ${esPeruanoText} nacionalidad peruana.`
		);
	};
...
(() => {
	...
	const obtenerTextoGamer = (esGamer: boolean) =>
		esGamer ? "me gusta" : "no me gusta";
	...
	const ejemploB = `Al sumar ${variableA} y ${variableB} nos da ${sumar(
		variableA,
		variableB
	)}.`;

	// Ejemplo de template literal usando string y boolean para un texto dinamico
	const ejemploC = `Hola, soy ${nombre} y ${obtenerTextoGamer(
		esGamer
	)} jugar videojuegos.`;
