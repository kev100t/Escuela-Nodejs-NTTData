...
export class NotLambdaService {
  private async getPdfFromPuppeter(request): Promise<Buffer> {
    ...
    const dirTemplate = join(`${__dirname}`,'..','template','proposal-business.html')
    const html = readFileSync(dirTemplate, 'utf-8');


    let parseTemplate = html.replace('{{senderCompany}}', request.senderCompany)
    parseTemplate = parseTemplate.replace('{{senderSlogan}}', request.senderSlogan)
    parseTemplate = parseTemplate.replace('{{clientCompany}}', request.clientCompany)
    parseTemplate = parseTemplate.replace('{{clientName}}', request.clientName)
    parseTemplate = parseTemplate.replace('{{proposalShortDescription}}', request.proposalShortDescription)
    parseTemplate = parseTemplate.replace('{{proposalNumber}}', request.proposalNumber)
    parseTemplate = parseTemplate.replace('{{proposalDate}}', request.proposalDate)
    parseTemplate = parseTemplate.replace('{{senderLocation}}', request.senderLocation)
    parseTemplate = parseTemplate.replace('{{proposalNumber}}', request.proposalNumber)
    parseTemplate = parseTemplate.replace('{{proposalDate}}', request.proposalDate)
    parseTemplate = parseTemplate.replace('{{senderLocation}}', request.senderLocation)
    parseTemplate = parseTemplate.replace('{{proposalIntroduction}}', request.proposalIntroduction)
    parseTemplate = parseTemplate.replace('{{projectScope}}', request.projectScope)
    ...
    for(const service of services.data) {
      tableData = tableData + '<tr><td>{{service}}</td><td>{{hours}}</td><td>{{price}}</td><td>{{total}}</td></tr>'
    ...
    parseTemplate = parseTemplate.replace('{{aditionalInformation}}', request.aditionalInformation)
    ...
    parseTemplate = parseTemplate.replace('{{senderEmail}}', request.senderEmail)
